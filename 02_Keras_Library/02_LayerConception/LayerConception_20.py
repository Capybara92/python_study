...
for epoch_idx in range(num_epochs):
    ...
    model.reset_states() # 다음 훈련할 데이터가 이전의 훈련할 데이터와 완전히 연관 없을 때 사용하는 것이다.
...
#순환 신경망 모델 중 가장 많이 사용되는 LSTM 모델에 대해서 알아보고, 주요 인자들이 어떤 특성을 가지고 있는 지도 살펴보았다.
#앞서 살펴본 4가지 모델에 대해서 학습 손실값을 그래프로 표시해봤다.
#다층퍼셉트론 모델 ->>> 기본 LSTM 모델 ->>> 상태유지 LSTM 모델 (1개 속성) ->>> 상태유지 LSTM 모델 (2개 속성) 순으로 더 빨리 학습되는 것을 확인할 수 있다.
